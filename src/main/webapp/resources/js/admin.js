/****** Admin Start ***********/
function activate(name, cid) {
	$("#activationModal .modal-info > span").text("A");
	$("#activationModal .modal-info > b > span").text(name);
	$("#activationModal .modal-footer > .btn-success").attr("onclick", "asyncProcess('actions/admin/clients/activation.php?activate=true', "+cid+", activateSuccess)");
	$("#activationModal").modal('show');
}
function activateSuccess(data, status) {
	var obj = JSON.parse(data);
	$("#activationModal").modal("hide");
	showSnackbar("activatedSnackbar");
	$("tr#for-"+obj.cid+" .act").html("<i class='fa fa-check text-success'></i>");
	var elem = $("tr#for-"+obj.cid+" .btn-success");
...
function deactivate(name, cid) {
	$("#activationModal .modal-info > span").text("Dea");
	$("#activationModal .modal-info > b > span").text(name);
	$("#activationModal .modal-footer > .btn-success").attr("onclick", "asyncProcess('actions/admin/clients/activation.php?activate=false', "+cid+", deactivateSuccess)");
	$("#activationModal").modal('show');
}
function deactivateSuccess(data, status) {
	var obj = JSON.parse(data);	
	$("#activationModal").modal("hide");
	showSnackbar("deactivatedSnackbar");
	$("tr#for-"+obj.cid+" .act").html("<i class='fa fa-remove text-danger'></i>");
	var elem = $("tr#for-"+obj.cid+" .btn-warning");
...
function kickout(name, cid) {
	$("#kickoutModal .modal-info > b > span").text(name);
	$("#kickoutModal .modal-footer > .btn-danger").attr("onclick", "asyncProcess('actions/admin/clients/kickout.php',"+cid+", kickoutSuccess)");
...
function kickoutSuccess(data, status) {
	...
	$("tr#for-"+obj.cid+"").hide("slow", function() {
		$("tr#for-"+obj.cid+"").remove();
	});
	showSnackbar("kickedoutSnackbar");
}
function makeAdmin(fname, cid) {
	$("#makeAdminModal .modal-info > b > span").text(fname);
	$("#makeAdminModal .modal-footer .btn-success").attr("onclick", "asyncProcess('actions/admin/clients/promote.php',"+cid+", makeAdminSuccess)");
...
function edit(fname, lname, email, gender, cont, dp, street, town, city, state, cid, add_id) {
	$("#clientEditModal .modal-body .user-dp-lg").attr("style","background-image: url('"+dp+"');");
	...
	$("#clientEditModal .modal-body #editForm").attr("action", "actions/admin/clients/update.php?cid="+cid);
	// $("#clientEditModal .modal-body #editForm").attr("onsubmit", "editClientAction('actions/admin/client-update.php', "+cid+")");
	$("#clientEditModal .modal-body #change-address").attr("onclick", "changeAddress('"+street+"', '"+town+"', '"+city+"', '"+state+"', "+add_id+")");
	$("#clientEditModal .modal-body #change-password").attr("onclick", "changePassword("+cid+")");
	$("#clientEditModal .modal-body .change-picture").attr("onclick", "changePicture('"+dp+"', '"+fname+" "+lname+"', "+cid+")");
...
function editClientSuccess(data, status) {
	...
	// $('#for-'+elem[0]+' .c-email').html("<a onclick=\"clientMail('"+elem[3]+"')\">"+elem[3]+"</a>");
...
function address(street, town, city, state, add_id) {
	// alert(street);
	$("#clientAddressModal table tbody").html("<tr><td>"+street+"</td><td>"+town+"</td><td>"+city+"</td><td>"+state+"</td></tr>");
	$("#clientAddressModal .modal-footer #change-address-2").attr("onclick", "changeAddress('"+street+"', '"+town+"', '"+city+"', '"+state+"', "+add_id+", 'client')");
	$("#clientAddressModal").modal('show');
}
function changePassword(cid) {
	// $("#clientEditModal").modal('hide');
	$("#clientChangePasswordModal form").attr("action", "actions/admin/clients/change-password.php?cid="+cid);
	$("#clientChangePasswordModal").modal('show');
}
function changePicture(dp, name, cid) {
	// $("#clientEditModal").modal('hide');
	$("#clientChangePictureModal .user-dp-xl").attr("style", "background-image: url('"+dp+"');");
	$("#clientChangePictureModal .user-dp-xl").attr("onclick", "showImageModal('"+name+"', '"+dp+"')");
	$("#clientChangePictureModal form").attr("action", "actions/admin/clients/profile.php?old_dp="+dp+"&cid="+cid);
...
function showPropAddress(street, town, city, state, add_id) {
	$("#propAddressModal table tbody").html("<tr><td>"+street+"</td><td>"+town+"</td><td>"+city+"</td><td>"+state+"</td></tr>");
	$("#propAddressModal .modal-footer #change-prop-address").attr("onclick", "changeAddress('"+street+"', '"+town+"', '"+city+"', '"+state+"', "+add_id+", 'prop')");
	$("#propAddressModal").modal('show');
}
function showPropFeatures(bhk, bath, age, furn, area, l_area, hosp, school, rail, pid) {
	...
	furnished += "'></i>"
	$("#propFeaturesModal table tbody").html("<tr><td>"+bhk+"</td><td>"+bath+"</td><td>"+age+" Years</td><td class='text-center'>"+furnished+"</td><td>"+hosp+" KMS.</td><td>"+school+" KMS</td><td>"+rail+" KMS</td><td>"+area+" Sq-Ft.</td><td>"+l_area+" Sq-Ft.</td></tr>");
	$("#propFeaturesModal #change-prop-features").attr("onclick", "editPropFeatures("+bhk+","+bath+","+age+","+furn+","+hosp+","+school+","+rail+","+area+","+l_area+", "+pid+")");
...
function showPropInfo(amens, units, floor, t_floors, desc, tnc, time, edit, pid) {
	...
	// $("#propInfoModal table #amen").html("<tr><td>"+amenities+"</td><td>"+units+"</td><td>"+floor+"</td><td>"+t_floors+"</td><td>"+desc+"</td><td>"+tnc+"</td><td>"+time+"</td><td>"+edit+"</td></tr>");
	...
	$("#propInfoModal #change-prop-info").attr("onclick", "editPropInfo('"+amens+"',"+units+","+floor+","+t_floors+",'"+desc+"','"+tnc+"',"+pid+")");
