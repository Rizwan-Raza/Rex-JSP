...
function readURLs(input) {
	...
	for (var i = files.length - 1; i >= 0; i--) {
		if (/^image/.test( files[i].type)) {
			...
			reader.onloadend = function(){
				$(".insert-here").append("<div class='col-xs-3 prop-image' id='image"+x+"'></div>");
				$(".insert-here #image"+x).css("background-image", "url("+this.result+")");
				x++;
			}
		}
	}
...
function changeAddress(street, town, city, state, add_id, type) {
	...
	if (type == "prop") {
		$("#changeAddressModal .modal-header h4").html("<i class='fa fa-home'></i> Property's Address");
		$("#changeAddressModal form").attr("action", "actions/cnp-change-address.php?type=prop&add_id="+add_id);
	} else {
		$("#changeAddressModal form").attr("action", "actions/cnp-change-address.php?type=client&add_id="+add_id);
	}
	$("#changeAddressModal").modal('show');
}
function editPropFeatures(bhk, bath, age, furn, hosp, school, rail, area, l_area, pid) {
	// alert(pid);
	...
	$("#changeFeaturesModal #changeFeatForm").attr("action", "actions/props/change-features.php?pid="+pid);
...
function editPropInfo(amens, units, floor, t_floors, desc, tnc, pid) {
	...
	if (amens != "'NULL") {
		...
		for (var i = 0; i < amenities.length; i++) {
			$("#changeInfoForm input[value='"+amenities[i]+"']").attr("checked","checked");
		}
	}
	...
	$("#changeInfoModal #changeInfoForm").attr("action", "actions/props/change-info.php?pid="+pid);
...
function showPropImages(srcs, title, pid) {
	...
	for (i = 0; i < num; i++) { 
		str += "<div class='col-md-"+div+" col-sm-"+div+"'> <div class='buy-prop-image image-triggerer' style=\"background-image: url('"+src_arr[i]+"')\" onclick=\"showImageModal('"+title+"', '"+src_arr[i]+"');\"><div class='remove-image diggle'><i class='fa fa-remove fa-fw'></i></div></div></div>";
	}
	$("#propImagesModal .row#prop-img-holder").html(str);
	$("#propImagesModal .modal-footer .pull-right p").html('<p>Change <a id="change-prop-images" onclick="addOrRemoveImages(\''+srcs+'\', \''+title+'\', '+pid+')">Images?</a></p>');
...
function addOrRemoveImages(srcs, title, pid) {
	...
	var num = src_arr.length;
	for (i = 0; i < num; i++) { 
		$("div[class*='col-md-']:nth-child("+(i+1)+") .buy-prop-image .remove-image").attr("onclick", "removeImage('"+src_arr[i]+"', '"+title+"')");
	}
	$(".buy-prop-image").removeClass("image-triggerer");
	$(".buy-prop-image").removeAttr("onclick");
	$(".buy-prop-image .remove-image").show();
	$("#propImagesModal .modal-footer .pull-right p").html('<p>Add <a id="change-prop-images" onclick="addImage('+pid+')">Image?</a></p>');
	// $("#propImagesModal .modal-footer .pull-right p").html('<input type="file">');
}
function removeImage(src, title) {
	...
	$("#removePropImageModal .modal-body .image-triggerer").css("background-image", "url('"+src+"')");
	$("#removePropImageModal .modal-body .image-triggerer").attr("onclick", "showImageModal('"+title+"', '"+src+"')");
	$("#removePropImageModal .modal-footer .btn").attr("onclick", "asyncProcess('actions/props/remove-image.php', '"+src+"', removeImageSuccess)");
...
function removeImageSuccess(data, status) {
	...
	$("#propImagesModal .buy-prop-image[style*=\"background-image: url('"+obj.src+"')\"]").parent().hide("slow", function() {
		$("#propImagesModal .buy-prop-image[style*=\"background-image: url('"+obj.src+"')\"]").parent().remove();
	});
	$("#my-property .buy-prop-image[style*=\"background-image: url('"+obj.src+"')\"]").parent().hide("slow", function() {
		$("#my-property .buy-prop-image[style*=\"background-image: url('"+obj.src+"')\"]").parent().remove();
	});
	var num = $(".buy-prop-image[style*=\"background-image: url('"+obj.src+"')\"").parent().parent().children().length;
	num /=2;
	var div = 12/num;
	switch (num) {
		case 1: case 3: 
	...
	$(".buy-prop-image[style*=\"background-image: url('"+obj.src+"')\"").parent().parent().children().attr("class", "col-md-"+div+" col-sm"+div);
	showSnackbar("propImageRemovedSnackbar");
}
function addImage(pid) {
	$("#addPropImageModal form").attr("action", "actions/props/add-image.php?pid="+pid);
...
function editProp(title, type, t_type, price, d_p, avail, pid) {
	...
	// $("#propEditModal #editPropForm").attr("onsubmit", "editPropAction('actions/props/edit-prop.php', "+pid+")");
	// $("#propEditModal #editPropForm").attr("onsubmit", "editPropAction('actions/props/test.php', "+pid+")");
	$("#propEditModal #editPropForm").attr("action", "actions/props/edit-prop.php?pid="+pid);
...
function deleteProp(name, pid) {
	...
	// $("#deletePropModal .modal-footer > .btn-danger").attr("onclick", "window.location.href='actions/props/delete-prop.php?pid="+pid+"'");
	$("#deletePropModal .modal-footer > .btn-danger").attr("onclick", "asyncProcess('actions/props/delete-prop.php',"+pid+", deletePropSuccess)");
...
function editRequest(type, city, state, bhk, bath, a_from, a_to, p_from, p_to, pr_id, by) {
	if (by == "client") {
	...
	$("#requestEditModal #editRequestForm").attr("action", "actions/props/edit-request.php?pr_id="+pr_id);
...
function deleteRequest(pr_id, type) {
	...
	$("#deleteRequestModal .modal-footer > .btn-danger").attr("onclick", "asyncProcess('actions/props/delete-request.php',"+pr_id+", deleteRequestSuccess)");
	$("#deleteRequestModal").modal('show');
}
function deleteRequestSuccess(data, status) {
	var obj = JSON.parse(data);
	if (obj.log == "admin") {
		$("#requires b#request-num").text((+$("#requires b#request-num").text()) - 1);
